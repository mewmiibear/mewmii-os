...
$form = [
    ...
    'instagram_username' => (string) $customer['instagram_username'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === '' && $form['email'] !== '') {
        $dupCheck = $pdo->prepare('SELECT COUNT(*) FROM customers WHERE LOWER(email) = LOWER(?) AND id != ?');
        $dupCheck->execute([$form['email'], $customerId]);
    ...
    if ($error === '') {
        ...
        try {
            $stmt = $pdo->prepare('
                UPDATE customers
                SET name = ?, email = ?, phone = ?, instagram_username = ?, birthday = ?, address = ?, notes = ?
                WHERE id = ?
            ');
            $stmt->execute([
                $form['name'],
            ...
            app_redirect('/modules/customers/edit.php?id=' . $customerId . '&updated=1');
