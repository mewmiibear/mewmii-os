...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $validated = asset_validate_form($pdo, $_POST);
    ...
    if ($error === '') {
        ...
        try {
            $assetId = asset_create($pdo, $validated['data'], $_SESSION['user_id'] ?? null);
            ...
            app_redirect('/modules/finance/assets.php?created=1');
        } catch (PDOException $exception) {
            $pdo->rollBack();
            // The UNIQUE index is the race-condition backstop behind asset_code_in_use()'s
            // check above - if two requests pass validation at the same moment, one lands here.
            $error = (string) $exception->getCode() === '23000'
                ? 'That asset code was just taken by another asset. Choose a different code.'
                : 'Failed to create asset: ' . $exception->getMessage();
        } catch (Exception $exception) {
            $pdo->rollBack();
            $error = 'Failed to create asset: ' . $exception->getMessage();
        }
    }
}
...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        ...
        <p class="page-description">Record equipment or furniture the business owns.</p>
    </div>
...
<form method="post" enctype="multipart/form-data" data-validate="1" novalidate>
    ...
    <div class="card p-4 mb-4">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" name="asset_code" maxlength="30" value="<?php echo app_escape($form['asset_code']); ?>" placeholder="Example: AST-MAC-001">
                <div class="form-text">Optional internal reference number.</div>
            </div>
            <div class="col-md-4">
                <label class="form-label">Name</label>
                <input type="text" class="form-control" name="name" maxlength="120" value="<?php echo app_escape($form['name']); ?>" required>
            ...
            <div class="col-12">
                ...
                <input type="text" class="form-control" name="description" maxlength="255" value="<?php echo app_escape($form['description']); ?>" required>
    ...
    <div class="card p-4 mb-4">
        ...
        <div class="row g-3">
            <div class="col-md-3">
                ...
                <input type="date" class="form-control" name="purchase_date" value="<?php echo app_escape($form['purchase_date']); ?>" required>
                <div class="invalid-feedback">Enter a valid date.</div>
            </div>
            <div class="col-md-3">
                <label class="form-label">Purchase Amount</label>
                <input type="number" step="0.01" min="0.01" class="form-control" name="purchase_amount" value="<?php echo app_escape($form['purchase_amount']); ?>" required>
            ...
            <div class="col-md-4">
                ...
                <input type="date" class="form-control" name="warranty_expiry" value="<?php echo app_escape($form['warranty_expiry']); ?>">
            </div>
        </div>
    </div>
    ...
    <div class="card p-4 mb-4">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <select class="form-select" name="assigned_to">
                    ...
                    <?php foreach ($users as $user): ?>
                        <option value="<?php echo (int) $user['id']; ?>" <?php echo $form['assigned_to'] === (string) $user['id'] ? 'selected' : ''; ?>>
            ...
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="location" maxlength="100" value="<?php echo app_escape($form['location']); ?>" placeholder="e.g. Office, Warehouse">
            </div>
            <div class="col-12">
    ...
    <div class="card p-4 mb-4">
        <h5 class="mb-3">Invoice / Document (optional)</h5>
        <input type="file" class="form-control" name="document" accept="image/jpeg,image/png,application/pdf">
        <p class="text-muted small mt-2 mb-0">JPG, PNG, or PDF - up to 8 MB. More can be attached later from the asset's own page.</p>
    </div>
    ...
    <div class="d-flex gap-2 py-3" style="position: sticky; bottom: 0; background: #fff; z-index: 1020; border-top: 1px solid #dee2e6;">
        ...
        <a class="btn btn-outline-secondary" href="/modules/finance/assets.php">Cancel</a>
    </div>
</form>
<?php
