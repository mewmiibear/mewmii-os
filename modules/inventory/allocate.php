...
// Purchase Planning link below (shown when there's demand but nothing arrived to allocate)
            ...
            <div class="mt-2 d-flex gap-1 flex-wrap">
                <?php foreach ($touchedOrders as $touchedOrder): ?>
                    <?php if ($canViewOrders): ?>
                        <a class="btn btn-sm btn-outline-success" href="/modules/orders/view.php?id=<?php echo (int) $touchedOrder['id']; ?>"><?php echo app_escape(order_display_number_compact($touchedOrder['order_number'])); ?></a>
                    <?php else: ?>
                        <span class="badge bg-success"><?php echo app_escape(order_display_number_compact($touchedOrder['order_number'])); ?></span>
                    <?php endif; ?>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
...
<?php if ($arrivedQuantity <= 0 && $candidates !== [] && $canManageSupplierOrders): ?>
    <div class="alert alert-warning">
        No arrived stock to allocate, but orders are waiting on this product -
                    ...
                    <tbody>
                        <?php foreach ($candidates as $candidate): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="/modules/orders/view.php?id=<?php echo (int) $candidate['order_id']; ?>"><?php echo app_escape(order_display_number_compact($candidate['order_number'])); ?></a>
