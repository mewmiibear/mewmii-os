...
/**
 * CLI-only alert generator for the Notification & Alert Center (Phase 9B).
 *
 * Usage:
 *   php cli/generate_alerts.php   Runs notification_generate_alerts() (includes/
 *                                  notifications.php) once and reports how many new
 *                                  notifications were created per type.
 *
 ...
 * has no scheduling logic of its own, it just runs one generation pass per invocation, same
 * shape as cli/wc_webhook_process.php.
 *
 * De-duplication happens inside notification_generate_alerts() itself (one unread notification
 * per type+reference_id at a time) - running this script as often as you like never creates
 * duplicate rows for a still-open alert.
 *
