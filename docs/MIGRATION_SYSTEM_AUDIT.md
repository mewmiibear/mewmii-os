...
> **Addendum (2026-08-03, updated 2026-08-04) — counts in the body below are the point-in-time figures from when this audit was written and are deliberately left unedited as the historical record.** Three migrations have been added since (`migrate_finance_phase_a/b/c.php`), bringing the total to **24**. All three were registered in `includes/system_health.php` — Phase A and B retroactively, after a Finance readiness audit found them missing (the exact drift failure mode §1 predicts), and Phase C in the same commit as the migration itself, which is now the standing rule.
>
> **Update (2026-08-04):** the two long-untracked `supplier_order*` scripts named in §2 are now registered too. `migrate_supplier_order_currency.php` was confirmed **already applied in production** by a read-only `INFORMATION_SCHEMA` check before registration — but nothing inside the app could have answered that, which is precisely §4's point. `migrate_supplier_order_purchase_number_unique.php` required a small extension: it creates **only a UNIQUE index**, which the detection array could not express, so a `unique_column` check type was added that matches by column + `NON_UNIQUE = 0` rather than by `INDEX_NAME` — necessary because the same constraint is auto-named `purchase_number` on a fresh `schema.sql` install but `idx_supplier_orders_purchase_number_unique` on a migrated one, and a name-based check would false-alarm on every fresh install. It was deliberately **not** added to `SYSTEM_HEALTH_INDEXES`, whose hardcoded attribution would have named `migrate_production_hardening.php` instead.
>
> **Coverage is now 22 of 24 scripts.** Only `migrate_sync_logs_index.php` and `migrate_webhooks.php` remain untracked — both are registerable with plain table/column checks whenever someone picks them up. This addendum is itself evidence for §4/§6's core argument: a hand-maintained detection array drifts by default, and catching it depends on someone happening to audit for it.
