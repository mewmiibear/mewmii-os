...
The same P&L calculation designed in `FINANCE_ACCOUNTING_ARCHITECTURE.md` §5/`FINANCE_DATABASE_DESIGN.md` §7, but with the period selector defaulted to a full tax year rather than the operational daily/weekly/monthly views. No new calculation — the same report, a different default filter, since a tax filing needs the yearly figure specifically. Where COGS includes freight/customs/import-tax/courier costs, this report reflects whichever classification is currently set in `finance_cost_classifications` (`FINANCE_ACCOUNTING_ARCHITECTURE.md` §8) — a business that later becomes SST-registered and reclassifies Import Tax accordingly sees that change reflected here automatically, without this report needing its own separate update.
